...
describe('lib/Logger.runQuery', async () => {
  const runTest = async (awsSdk, params, expectCbWithError,
    expectCbWithData) => {
    await Logger.runQuery(awsSdk, params);
    
  };
  test('calling with right params', async () => {
    const rightParams = 'correct params';
    const realParams = 'correct params';
    let calledWithError = false;
    let calledWithData = false;
    const cb = (e, d) => {
      if(e) {
          ...
          constructor() {
            return {
              transactWrite: (params, cb) => {
                console.log('transactWrite called');
                console.log(`params: ${params}`);
                if(params === rightParams) {
                  console.log('params are right');
                  cb(false, true);
                }
                else {
                  console.log('params are not right');
                  cb(true, false);
                }
              }
            }
    ...
    const dbResult = await Logger.runQuery(awsSdk, realParams);
    expect(dbResult).toEqual({err: false, data: true});

  });
  test('calling with wrong params', () => {
