...
describe('checkJwt', () => {
  ...
  test.only("should refresh access token, new access token valid", async () => {
    const req = {};
    const sendStatus = jest.fn();
    const send = jest.fn();
    const status = jest.fn(() => {send});
    ...
    const jwt = {
      ...
      verify: jest.fn((token, secretOrPublicKey, options) => {
        if(secretOrPublicKey !== jwkPem || !options ||
          options.algorithm !== "RS256") {
          throw "mock verification failed";
        }
        if(token === access_token) {
          console.log("toxen expired");
          throw {
        ...
        if(token === refreshedTokens.access_token) {
          return;
        }
      })
    };
    const verifyJwt = jest.fn(() => {
      console.log("verifying jot...");
      return decodedJot.payload.sub
    });
    const next = jest.fn(() => {console.log("calling next...")});
    const middleware = checkJwt({
      jwk2Pem,
      jwt,
      tokenType: "access",
