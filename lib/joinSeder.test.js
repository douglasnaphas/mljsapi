...
describe('joinSeder', () => {
  ...
  const runTest = async ({req, expectedStatus, expectedResponseData,
    expectedCookie, randomStringGenerator, expectNext}) => {
  ...
  test('good code, 1st person to empty seder', () => { // 1
    return runTest({req: reqGoodCodeFreeNameEmptySeder,
      expectNext: true,
  ...
  test('good code, free name, crowded seder', () => { // 2
    return runTest({req: reqGoodCodeFreeNameCrowdedSeder,
      expectNext: true,
  ...
  test('good code, taken name, 1 other person at seder', () => { // 3
    return runTest({req: reqGoodCodeTakenNameOnePersonSeder,
      expectedStatus: 400,
  ...
  test('good code, taken name, crowded seder', () => { // 4
    return runTest({req: reqGoodCodeTakenNameCrowdedSeder,
      expectedStatus: 400,
      expectedResponseData: apiResponseNameTaken,
      randomStringGenerator: emptyStringGenerator});
  });
  ...
  test('bad code, free name, crowded seder', () => { // 6
    return runTest({req: reqBadCodeFreeNameCrowdedSeder, expectedStatus: 400});
  });
  test('bad code, taken name, 1 person at seder', () => { // 7
    return runTest({req: reqBadCodeTakenNameOnePersonSeder,
      expectedStatus: 400});
  });
  test('bad code, taken name, crowded seder', () => { // 8
    return runTest({req: reqBadCodeTakenNameCrowdedSeder, expectedStatus: 400});
  });
