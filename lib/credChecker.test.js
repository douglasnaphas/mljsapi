...
describe('credChecker, authorization middleware', () => {
  ...
  test.skip('valid tokens in cookies, should advance, populate user', async () => {
    const req = {
      get: header => {
        return undefined;
      },
  ...
  test('no tokens in header/query, invalid in cookies, should get 401', async () => {
    ...
    const res = {
      ...
      send: () => {
        ...
        if (status401Set && !nextCalled) {
          console.log('setting status401Sent to true');
          status401Sent = true;
        }
      }
    };
    ...
    try {
      ...
      console.log('in try post await');
    } catch (e) {
      console.log('in catch');
    }
    console.log('**** status401Sent: ' + status401Sent);
