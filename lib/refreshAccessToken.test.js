...
describe("refreshAccessToken", () => {
  ...
  test("should use the refresh token to get a new access token 1", async () => {
    ...
    axios.post.mockImplementation((url, data, config) => {
      ...
      if(data !== qs.stringify({
        "grant_type": "refresh_token",
        "client_id": Configs.CognitoClientID(),
        "refresh_token": refreshToken
      })) {
        throw `config.data not stringified, got ${data}`
      }
  ...
  test("should use the refresh token to get a new access token 2",
    ...
    axios.post.mockImplementation((url, data, config) => {
      ...
      if(data !== qs.stringify({
        ...
        throw `config.data not stringified, got ${config.data}`
      }
      const parsedData = qs.parse(data);
      if(parsedData["grant_type"] !== `refresh_token`) {
        throw `wrong grant_type, expect refresh_token, got ` +
  ...
  test.skip("failed post",
    ...
    axios.post.mockImplementation((url, config) => {
      ...
      const parsedData = qs.parse(config.data);
      if(parsedData["grant_type"] !== `refresh_token`) {
        throw `wrong grant_type, expect refresh_token, got ` +
          `${parsedData["grant_type"]}`;
      }
