...
describe('lib/scriptMiddleware/s3Params', () => {
  const runTest = ({path, version, expectedS3Params, expectedStatus, expectNext,
    expectedData}) => {
  ...
  test('happy path 1', () => {
    ...
    runTest({path: path, version: version, expectNext: true, expectedS3Params:
      expectedS3Params});
  });
  test('happy path 2', () => {
  ...
  test('missing path', () => {
    ...
    runTest({version: version, expectedStatus: 500, expectedData:
      responses.SERVER_ERROR});
  });
  test('missing version', () => {
    const path = 'apathy';
    runTest({path: path, expectedStatus: 500, expectedData:
      responses.SERVER_ERROR});
  });
});