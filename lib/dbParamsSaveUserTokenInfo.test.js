...
describe('dbParamsSaveUserTokenInfo', () => {
  const runTest = ({locals, expectedDbParams, expectNext,
    ...
    if(expectedDbParams) {
      const {
        [schema.USER_NICKNAME]: expectedNickname,
        [schema.USER_EMAIL]: expectedEmail,
        sub: expectedSub,
        ["cognito:username"]: expectedCUn
      } = expectedDbParams.Item;
      const {
        [schema.USER_NICKNAME]: actualNickname,
        [schema.USER_EMAIL]: actualEmail,
        sub: actualSub,
        ["cognito:username"]: actualCUn,
        is_user_info: actualIsUserInfo
      } = res.locals.dbParamsSaveUserTokenInfo.Item;
      expect(actualNickname).toEqual(expectedNickname);
      expect(actualEmail).toEqual(expectedEmail);
      expect(actualSub).toEqual(expectedSub);
      expect(actualCUn).toEqual(expectedCUn);
      console.log(actualNickname)
      console.log(expectedNickname)
      expect(actualIsUserInfo).toBeGreaterThan(0);
    }
    if(expectNext) {
