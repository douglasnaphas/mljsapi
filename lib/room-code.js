...
function roomCode(awsSdk, roomCodeGenerator, configs) {
  ...
  const f = async (req, res) => {
    ...
    let success;
    success = false;
    const attempts = configs.roomCodeRetries(); // TODO: Store this in Configs and test what happens
                            // when attempts are exhausted, low priority
    
    for(let i = 0; ! success && i < attempts; i++) {
      code = roomCodeSequence.next().value; /*'AAAAAF';*/
      const now = new Date();
      params = {
        TableName: 'seders',
        Item: {
