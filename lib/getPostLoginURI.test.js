describe("getPostLoginURI", () => {
  ...
  test.each`
    ...
    ${"development"} | ${"http"}  | ${"localhost"}        | ${"ab"}            | ${"cd@e.com"}              | ${"http://localhost:3000?nickname=ab&email=cd%40e.com#/logging-in"}
    ...
    ${"development"} | ${"https"} | ${"localhost"}        | ${"ab1"}           | ${"cd@e.com"}              | ${"https://localhost:3000?nickname=ab1&email=cd%40e.com#/logging-in"}
    ...
    ${"production"}  | ${"http"}  | ${"api.passover.lol"} | ${"Mizz Ayaaff"}   | ${"cd@ex.com"}             | ${"https://passover.lol?nickname=Mizz%20Ayaaff&email=cd%40ex.com#/logging-in"}
  `("...", ({ nodeEnv, protocol, host, nickname, email, expected }) => {
    const req = {
      get: jest.fn(val => {
